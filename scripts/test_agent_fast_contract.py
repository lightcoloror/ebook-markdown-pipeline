...
def assert_agent_batch_results_inspection(tmpdir: Path) -> None:
    ...
    if inspected.get("contract_validation", {}).get("ok") is not True:
        raise AssertionError(f"Expected contract validation passthrough in agent batch inspection: {inspected}")
    ...
    expected_synthesized_actions = {"read_run_summary", "build_agent_handoff_bundle", "inspect_failed_artifacts", "inspect_review_items", "read_quality_comparison_json"}
...
def assert_agent_handoff_bundle_tool(tmpdir: Path) -> None:
    ...
    recommended = bundle.get("recommended_next_action") or {}
    if recommended.get("tool") != "read_artifact" or (recommended.get("arguments") or {}).get("artifact_type") != "markdown":
        raise AssertionError(f"Expected executable recommended handoff action: {generated}")
...
def assert_agent_batch_results_listing(tmpdir: Path) -> None:
    ...
    if listed["items"][0].get("contract_validation", {}).get("ok") is not True:
        raise AssertionError(f"Expected contract validation in listed batch: {listed}")
