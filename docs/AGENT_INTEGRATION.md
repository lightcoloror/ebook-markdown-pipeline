...
When taking over a previous batch, call `inspect_agent_batch_results` with the prior `agent-batch-results.json`. It returns the summary, quality comparison status, `recommended_rerun`, review items, and artifact paths without requiring the agent to parse the whole JSON by hand.
