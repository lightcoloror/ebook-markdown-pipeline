...
- Unstract：AGPL-3.0 平台项目，不建议作为本项目依赖混入。
- kreuzberg：Elastic License 2.0，属于 source-available 约束，不作为默认开源依赖。
...
## 本地源码审计结果（第三批：矩阵旁支候选补齐）

本节补齐 2026-06-13 反向检查中发现的旁支候选。它们多是“可替代基础库、专项工具或完整平台”，不一定进入下一阶段开发，但既然出现在矩阵中，就记录源码级边界，避免未来会话重复调研。
...
| Mammoth | `1cac245` | Python 包/CLI `mammoth`；DOCX 到 HTML 结构转换，模块含 `mammoth.docx`、`mammoth.html` | BSD-2-Clause | DOCX 轻量转换 | 可作为 Docling/MarkItDown 的 DOCX fallback 候选；目前优先不增加单格式依赖。 |
| RAGFlow / DeepDoc | `f671e7c` | RAG 平台，含 `deepdoc/parser/*`、`deepdoc/vision/*`、layout/OCR/table structure recognizer；Docker/API/CLI 多入口 | Apache-2.0；模型和完整平台依赖另查 | DeepDoc 版面/OCR 思路、RAG 产品闭环参考 | 不直接整合。DeepDoc 很相关，但 RAGFlow 是完整平台，先只作为结构/视觉模块参考。 |

### 不纳入源码整合候选的旁支名称

- LibreOffice headless：作为外部办公套件/命令行转换器思路存在，不作为本项目源码级整合候选；若未来需要，只通过用户已安装程序或容器调用。
- Jina Reader、Mathpix、LlamaParse、Mistral OCR、Azure Document Intelligence、Google Document AI、Amazon Textract、ABBYY、Reducto：属于在线服务或闭源/云端对标，不按开源源码审计处理；后续只通过统一 provider 抽象接入。
...
| DOCX / PPTX / XLSX / HTML / CSV | Docling 可选后端，Pandoc fallback | MarkItDown、Unstructured、Mammoth；LibreOffice 仅作为外部程序思路 | Docling 继续做主结构化后端；MarkItDown 可作为更轻的可选后端。 |

### PDF 解析与结构增强

| 模块 | 已整合项目/能力 | 未整合候选 | 下一步建议 |
| --- | --- | --- | --- |
| PDF fast path | PyMuPDF、PyMuPDF4LLM、自研 PyMuPDF text fallback、pdfplumber diagnostics | pypdf、pdfminer.six、PDFBox（均已源码审计） | pdfplumber 已进入诊断层；pypdf/pdfminer 可做轻量兜底。 |
| 复杂 PDF / 结构化 PDF | MinerU、Marker、Docling 可选管道 | RAGFlow DeepDoc、Surya、MegaParse（均已源码审计） | 先用真实样本压测现有三条重管道，再决定是否接新后端。 |
...
| 模块 | 已整合项目/能力 | 未整合候选 | 下一步建议 |
| --- | --- | --- | --- |
| Web archive 视觉复查 | 自研 `process_web_archive` + 复用 `web-content-fetcher` archive 产物 | Crawl4AI、Trafilatura；Jina Reader 为在线对标 | 网页抓取/下载/归档统一放在 `web-content-fetcher`；本项目只做 archive 的视觉复查和图文补强。 |
